ss = """oaoe rxeq vssdqtu xrk cjv yaoqp loo
...
# ss = """a really fun test
# something that is taht"""
...
def is_valid2(phrase):
    ...
    new_a = list(map(lambda x: ''.join(sorted(x)), a))
    # print(new_a)

    if len(set(new_a)) == len(a):
        return True
    else:
        return False
...
def main():
    the_count = len(list(filter(lambda x: is_valid(x), ss.splitlines())))
    print("number of valid pass phrases is {}".format(the_count))

    the_count2 = len(list(filter(lambda x: is_valid2(x), ss.splitlines())))
    print("number of super valid pass phrases is {}".format(the_count2))
